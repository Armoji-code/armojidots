...
acc   = colorsys.hsv_to_rgb(h, s, v)
dim   = colorsys.hsv_to_rgb(h, s, v * 0.68)
bg    = colorsys.hsv_to_rgb(h, s * 0.80, 0.13)   # deep tinted background
bg2   = colorsys.hsv_to_rgb(h, s * 0.65, 0.25)   # raised surface / unfocused border
fg    = colorsys.hsv_to_rgb(h, s * 0.18, 0.94)   # near-white, tinted
muted = colorsys.hsv_to_rgb(h, s * 0.40, 0.62)
...
for dh, ds, dv in spec:
    regs.append(colorsys.hsv_to_rgb((h + dh) % 1.0, min(1.0, s * ds), dv))
    brights.append(colorsys.hsv_to_rgb((h + dh) % 1.0, min(1.0, s * ds * 0.9), min(1.0, dv + 0.12)))
